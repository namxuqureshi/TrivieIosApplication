...
//  BingLine
//
//  Created by Shahid Saleem on 2/27/21.
//
...
extension Date {
    ...
    func timeAgoDisplay() -> String {
        ...
        let currentDt = Date().toLocalTime()
        let minuteAgo = calendar.date(byAdding: .minute, value: -1, to: currentDt)!
        let hourAgo = calendar.date(byAdding: .hour, value: -1, to: currentDt)!
        let dayAgo = calendar.date(byAdding: .day, value: -1, to: currentDt)!
        let weekAgo = calendar.date(byAdding: .weekOfYear, value: -1, to: currentDt)!
        let monthAgo = calendar.date(byAdding: .month, value: -1, to: currentDt)!
        let yearAgo = calendar.date(byAdding: .year, value: -1, to: currentDt)!
        switch self.compare(minuteAgo){
        case .orderedDescending,.orderedSame:
            let diff = Calendar.current.dateComponents([.second], from: self, to: currentDt).second ?? 0
            return "\(diff)" + (diff == 1 ? "s" : "s")
        case .orderedAscending:
            switch self.compare(hourAgo){
            case .orderedDescending,.orderedSame:
                let diff = Calendar.current.dateComponents([.minute], from: self, to: currentDt).minute ?? 0
                return "\(diff)" + (diff == 1 ? "m" : "m")
            case .orderedAscending:
                switch self.compare(dayAgo){
                case .orderedDescending,.orderedSame:
                    let diff = Calendar.current.dateComponents([.hour], from: self, to: currentDt).hour ?? 0
                    return "\(diff)" + (diff == 1 ? "h" : "h")
                case .orderedAscending:
                    switch self.compare(weekAgo){
                    case .orderedDescending,.orderedSame:
                        let diff = Calendar.current.dateComponents([.day], from: self, to: currentDt).day ?? 0
                        return "\(diff)" + (diff == 1 ? "d" : "d")
                    case .orderedAscending:
                        switch self.compare(monthAgo){
                        case .orderedDescending,.orderedSame:
                            let diff = Calendar.current.dateComponents([.weekOfYear], from: self, to: currentDt).weekOfYear ?? 0
                            return "\(diff)" + (diff == 1 ? "w" : "w")
                        case .orderedAscending:
                            switch self.compare(yearAgo){
                            case .orderedDescending,.orderedSame:
                                let diff = Calendar.current.dateComponents([.month], from: self, to: currentDt).month ?? 0
                                return "\(diff)" + (diff == 1 ? "month" : "months")
                            case .orderedAscending:
                                let diff = Calendar.current.dateComponents([.year], from: self, to: currentDt).year ?? 0
                                return "\(diff)" + (diff == 1 ? "year" : "year")
                            }
                        }
                    }
                }
...
//            return "\(diff) " + (diff == 1 ? "il y a sec" : "il y a secondes")
//        } else if hourAgo < self {
//            let diff = Calendar.current.dateComponents([.minute], from: self, to: Date()).minute ?? 0
//            return "\(diff) " + (diff == 1 ? "il y a min" : "il y a quelques minutes")
//        } else if dayAgo < self {
//            let diff = Calendar.current.dateComponents([.hour], from: self, to: Date()).hour ?? 0
//            return "\(diff) " + (diff == 1 ? "Une heure avant" : "il y a des heures")
//        } else if weekAgo < self {
//            let diff = Calendar.current.dateComponents([.day], from: self, to: Date()).day ?? 0
//            return "\(diff) " + (diff == 1 ? "il y a jour" : "il y a quelques jours")
//        }
//        let diff = Calendar.current.dateComponents([.weekOfYear], from: self, to: Date()).weekOfYear ?? 0
//        return "\(diff) " + (diff == 1 ? "il y a une semaine" : "il y a des semaines")
    }
}
...
extension UIImage {
    func setIconSize(scaledToSize newSize: CGSize) -> UIImage {
